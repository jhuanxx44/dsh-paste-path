...
function rejectUnlessLocal(req, res) {
  if (!isLoopbackAddress(req.socket.remoteAddress)) {
    sendJson(res, 403, {
      error: 'dsh-paste-path reads the clipboard of the machine running `dsh web`; remote browsers are not supported.',
...
function registerRoutes(ctx) {
  ...
  // webServer only becomes visible to ctx.get() once its fiber turns active —
  // after its async listen resolves — and the loader starts entries
  // concurrently, so a one-shot ctx.get() in apply() can silently lose that
  // race and never register. ctx.inject defers this body until the service is
  // available and re-runs it whenever the service goes away and comes back.
