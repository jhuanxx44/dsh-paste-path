  ...
  factory: (require) => {
    ...
    async function requestJson(path, options) {
      ...
      try {
        ...
        const error = new Error('dsh-paste-path returned HTTP ' + response.status)
        error.status = response.status
        throw error
      }
      if (!response.ok) {
        const error = new Error(typeof value.error === 'string' ? value.error : 'dsh-paste-path returned HTTP ' + response.status)
        error.status = response.status
    ...
    function disableRemote() {
      ...
      if (typeof console !== 'undefined' && typeof console.info === 'function') {
        console.info('dsh-paste-path: disabled — `dsh web` is not running on this machine, and a remote host\'s clipboard cannot be pasted.')
      }
    }
    ...
    function refreshPeek() {
      ...
      requestJson(PEEK_ROUTE, { method: 'GET' }).then((result) => {
        ...
        peekInFlight = false
        if (error && (error.code === 'remote-not-supported' || error.status === 403)) disableRemote()
      })
    }

    function draftHasPath(draft, path) {
      if (!draft) return false
      const lines = draft.split(/\r?\n/)
      for (const line of lines) {
        if (line.trim() === path) return true
      }
      return draft.indexOf(path) !== -1
    }

    function appendDraft(inputActions, draft, nextPaths) {
      const fresh = []
      for (const path of nextPaths) {
        if (!draftHasPath(draft, path)) fresh.push(path)
      }
    ...
    function pastePaths(actions, draft, phase) {
      ...
      requestJson(PASTE_ROUTE, { method: 'POST' }).then((result) => {
        ...
        const inserted = appendDraft(actions, draft, nextPaths)
        applyPeek({ ready: true })
        if (inserted === 0) {
          showToast({ error: false, text: '这些路径已经在输入框里了' })
          return
        }
        showToast({
          error: false,
